...
def classify_LLM(input_text):
    llm = ChatUpstage(model="solar-pro")
    prompt_template = PromptTemplate.from_template(
        """
        {input_text}

        Based on the options (back pain, neck pain, lower extremity pain, upper extremity pain, none of above—only consider physical pain related to muscles, joints, or bones), what is my issue? Please respond with only one or more of the given terms, separated by commas if necessary, without a period.
        """
    )
    chain = prompt_template | llm | StrOutputParser()
    chain = chain | (lambda x: x.strip())
...
def LLM():
    try:
        data = request.json
        print("Received /LLM request with data:", data)
        input_text = data.get('input_text', '')
        result = classify_LLM(input_text)
