    ...
    const translateText = async (text, targetLanguage) => {
        ...
        try {
            const response = await axios.post(
                ...
                {
                    model: 'gpt-4o-mini',
                    messages: [{role: 'user', content: `Translate the following text to ${targetLanguage}. If it is already in ${targetLanguage}, don't translate:\n\n${text}`}], //
                    temperature: 0
                },
                {
                    headers: {
        ...
        try {
            ...
            if (currentStep ==='initial'){
                if (!userLanguage) {
                    ...
                    const translatedMessage = await translateText(`Language set to ${input}. Press any keys to continue.`, 'english');
                    setTranslatedMessages([...messages, { text: translatedMessage, user: false }]);
                }
                setCurrentStep('initial_2')
            }
            if (currentStep === 'initial_2') {
                const translatedMessage = await translateText("Do you have a diagnostic assessment? (yes/no)", userLanguage);
                setTranslatedMessages([...messages, { text: translatedMessage, user: false }]);
                //setMessages([...messages, { text: "Do you have a diagnostic assessment? (yes/no)", user: false }]);
                response = await axios.post('/check_diagnosis', { has_diagnosis: input }); //https://reconect-ef4dd30ce7be.herokuapp.com
                if (input.toLowerCase() === 'no') {
                    console.log(response.data.hasExistingInfo);
                    if (response.data.hasExistingInfo) {
                        const translatedMessage = await translateText("Found existing patient information. Would you like to use it? (yes/no)", userLanguage);
                        setTranslatedMessages([...messages, { text: translatedMessage, user: false }]);                        
                        ...
                        const translatedMessage = await translateText("No existing patient information found. \n (Q0)\nDoctor: What is your chief complaint?\nPatient: ", userLanguage);
                        setTranslatedMessages([...messages, { text: translatedMessage, user: false }]);  
                        setCurrentStep('userInput');
                    } 
                } else if (input.toLowerCase() === 'yes') {
                    setCurrentStep('Rehab');
                    const translatedMessage = await translateText("Please enter the diagnostic assessment ID: ", userLanguage);
                    setTranslatedMessages([...messages, { text: translatedMessage, user: false }]);  
                }
            } 
            if (currentStep === 'awaitingExistingInfoResponse') {
                if (input.toLowerCase() === 'yes' || input.toLowerCase() === '예') {
                    response = await axios.post('/use_existing_info'); //https://reconect-ef4dd30ce7be.herokuapp.com
                    const [translatedMessage_1, translatedMessage_2, translatedMessage_3] = await Promise.all([
                        translateText(input, userLanguage),
                        translateText(response.data.diagnosis, userLanguage),
                        translateText("All questions completed. Send any keys to restart.", userLanguage)
                    ]);
                    setTranslatedMessages([...messages, { text: translatedMessage_1, user: true },{ text: translatedMessage_2, user: false },{ text: translatedMessage_3, user: false }]);
                    setCurrentStep('complete');
                } else if (input.toLowerCase() === 'no'|| input.toLowerCase() === '아니오') {
                    setCurrentStep('userInput');
                    const translatedMessage = await translateText("(Q0)\nDoctor: What is your chief complaint?\nPatient: ", userLanguage);
                    setTranslatedMessages([...messages, { text: translatedMessage, user: false }]);
                }             
            } 
            if (currentStep === 'userInput') {
                if (input.toLowerCase()==='lower extremity pain'|| input.toLowerCase()==='하지통'){
                    ...
                    const translatedMessage = await translateText("Invalid input. Please try again.", userLanguage);
                    setTranslatedMessages([...messages, { text: translatedMessage, user: false }]);
                }
                const translatedMessage = await translateText("Press any keys to proceed. ", userLanguage);
                setTranslatedMessages([...messages, { text: translatedMessage, user: false }]);
            }
            if (currentStep === 'userInput_Lower_Extremity'){
                ...
                : ReactDOMServer.renderToStaticMarkup(currentQuestion.question);
                if (currentQuestion.validator(input)) {
                    ...
                    if (Object.keys(newPatientInfo).length === questions_Lower_Extremity.length) {
                        response = await axios.post('/user_input', { patient_info: newPatientInfo }); //https://reconect-ef4dd30ce7be.herokuapp.com
                        const [translatedMessage_1, translatedMessage_2, translatedMessage_3] = await Promise.all([
                            translateText(input, userLanguage),
                            translateText(response.data.diagnosis, userLanguage),
                            translateText("All questions completed. Send any keys to restart.", userLanguage)
                        ...
                        const [translatedMessage_1, translatedMessage_2] = await Promise.all([
                            translateText(input, userLanguage),
                            translateText(questionText, userLanguage)
                        ]);
                        setTranslatedMessages([...messages, { text: translatedMessage_1, user: true }, { text: translatedMessage_2, user: false }]);
                    }
                } else {
                    const translatedMessage = await translateText("Invalid input. Please try again.", userLanguage);
                    setTranslatedMessages([...messages, { text: translatedMessage, user: false }]);
            ...
            if (currentStep === 'Rehab') {
                response = await axios.post('/Check_diagnosis_id', { diagnosis_id: input }); //https://reconect-ef4dd30ce7be.herokuapp.com
                ...
                const [translatedMessage_1, translatedMessage_2] = await Promise.all([
                    translateText(input, userLanguage),
                    translateText(response.data.result, userLanguage)
                ]);
                setTranslatedMessages([...messages, { text: translatedMessage_1, user: true }, { text: translatedMessage_2, user: false }]);
            }
            if (currentStep === 'File_exists') {
                if (input.toLowerCase() === 'yes') {
                    response = await axios.post('/File_exists', {'diagnosis_id': diagnosis_id}); //https://reconect-ef4dd30ce7be.herokuapp.com
                    ...
                    const [translatedMessage_1, translatedMessage_2] = await Promise.all([
                        translateText(input, userLanguage),
                        translateText(response.data.result_str, userLanguage)
                    ]);
                    setTranslatedMessages([...messages, { text: translatedMessage_1, user: true }, { text: translatedMessage_2, user: false }]);
                } else {
                    setCurrentStep('File_not_exists');
                    const [translatedMessage_1, translatedMessage_2] = await Promise.all([
                        translateText(input, userLanguage),
                        translateText("Press any keys to write new Patient info.", userLanguage)
                    ]);
                    setTranslatedMessages([...messages, { text: translatedMessage_1, user: true }, { text: translatedMessage_2, user: false }]);
                }       
            }
            if (currentStep === 'File_not_exists') {
                ...
                response = await axios.post('/Update_patient_info', { 'patient_info': patient_info_temp, 'diagnosis_id': diagnosis_id}) //https://reconect-ef4dd30ce7be.herokuapp.com

                const [translatedMessage_1, translatedMessage_2, translatedMessage_3] = await Promise.all([
                    translateText(input, userLanguage),
                    translateText(response.data.result, userLanguage),
                    translateText("Would you like to use these diagnosis and disabilities?", userLanguage)
                ]);
                setTranslatedMessages([...messages, { text: translatedMessage_1, user: true }, { text: translatedMessage_2, user: false }, { text: translatedMessage_3, user: false }]);
                setCurrentStep('File_exists');  
            }
            if (currentStep === 'Compare_item_scores') {
                response = await axios.post('/Input_item_scores', { 'diagnosis_id': diagnosis_id}); //https://reconect-ef4dd30ce7be.herokuapp.com
                ...
                if (Array.isArray(decreased_items) && decreased_items.every(item => item === "")) {
                    ...
                    const [translatedMessage_1, translatedMessage_2, translatedMessage_3] = await Promise.all([
                        translateText(input, userLanguage),
                        translateText(response.data.result, userLanguage),
                        translateText("Now let me screen possible complications.\n(Q1)\nDoctor: How long has it been since you were diagnosed with stroke?\nPatient: ", userLanguage)
                    ]);
                    setTranslatedMessages([...messages, { text: translatedMessage_1, user: true }, { text: translatedMessage_2, user: false }, { text: translatedMessage_3, user: false }]);
                } else {
                    ...
                    const [translatedMessage_1, translatedMessage_2, translatedMessage_3] = await Promise.all([
                        translateText(input, userLanguage),
                        translateText(response.data.result, userLanguage),
                        translateText("Press any keys to see rehabilitation training for decreased items.", userLanguage)
                    ]);
                    setTranslatedMessages([...messages, { text: translatedMessage_1, user: true }, { text: translatedMessage_2, user: false }, { text: translatedMessage_3, user: false }]);
                }
            }
            if (currentStep === 'Get_item_scores_temp') { 
                const translatedMessages = await Promise.all(
                    emptyDisabilities.map(async (disability) => {
                        const translatedMessage = await translateText(disabilities_descriptions.find(q => q.key === disability).question, userLanguage);
                        return { text: translatedMessage, user: false };
                    })
                );
                ...
                const translatedMessage2 = await translateText("Press any keys to proceed", userLanguage);
                setTranslatedMessages(prevMessages => [
                    ...prevMessages, 
                    { text: translatedMessage2, user: false }
                ]);
            ...
            if (currentStep ==='Get_item_scores') {
                ...
                response = await axios.post('/Get_item_scores', { 'emptyDisabilities': emptyDisabilities, 'answers': answers, 'diagnosis_id': diagnosis_id}) //https://reconect-ef4dd30ce7be.herokuapp.com
                setCurrentStep('Compare_item_scores');

                const [translatedMessage_1, translatedMessage_2, translatedMessage_3] = await Promise.all([
                    translateText(input, userLanguage),
                    translateText(response.data.result, userLanguage),
                    translateText("Press any keys to compare today's score with last 7-day's score.", userLanguage)
                ]);
                setTranslatedMessages([...messages, { text: translatedMessage_1, user: true },{ text: translatedMessage_2, user: false },{ text: translatedMessage_3, user: false }]);
            }
            if (currentStep === 'RehabTraining') {
                response =await axios.post('/Get_rehabilitation_evaluation', { 'decreased_items': decreased_items, 'disabilities': disabilities}) //https://reconect-ef4dd30ce7be.herokuapp.com
                const [translatedMessage_1, translatedMessage_2, translatedMessage_3] = await Promise.all([
                    translateText(input, userLanguage),
                    translateText(response.data.result, userLanguage),
                    translateText("Now let me screen possible complications. Press any keys to proceed.", userLanguage)
                ]);
                setTranslatedMessages([...messages, { text: translatedMessage_1, user: true },{ text: translatedMessage_2, user: false },{ text: translatedMessage_3, user: false }]);                
                setCurrentStep('ComplicationsInput_temp');                
            }
            ...
            if (currentStep === 'ComplicationsInput_TBI') {
                    ...
                    if (currentQuestion.validator(input)) {
                            ...
                            if (Object.keys(newPatientInfo).length === complication_questions_Traumatic_Brain_Injury.length) {
                                response = await axios.post('/complications_input', { patient_info: newPatientInfo, diagnosis: diagnosis }); https://reconect-ef4dd30ce7be.herokuapp.com
